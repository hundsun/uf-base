 ...
 ************************************************************/
	...
	function main(/*Map<Object, Object> */ context) {
		...
		var sqlFileName = "user_hserror_or.sql"; //文件名
		...
		if(genMode=="install"){
			...
			if(errornoCategorys.length > 0){
				errornoInfoBuffer.append("\r\n\r\nprompt Create errormessage InitValue ...\n");
				errornoInfoBuffer.append("\r\n");
				errornoInfoBuffer.append("begin\r\n");
			 	errornoInfoBuffer.append("  execute immediate 'truncate table hs_user.errormsg';\r\n");
			  	errornoInfoBuffer.append("\r\n");
			   ...
			   for ( var i in errornoCategorys) {
				...
				for ( var j in categoryIems) {
					...
					if (stringutil.isNotBlank(name)) {
						errornoInfoBuffer.append("declare v_rowcount number(5);\r\n");
						errornoInfoBuffer.append("begin\r\n");
						errornoInfoBuffer.append("  select count(*) into v_rowcount from dual\r\n");
						errornoInfoBuffer.append("    where exists (select 1 from hs_user.errormsg where error_no = ").append(errorNo).append(");\r\n");
